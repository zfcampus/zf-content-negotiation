...
class ContentTypeListenerTest extends TestCase
{
    ...
    public function testReturnsArrayWhenFieldNamesHaveArraySyntaxWithFileInput()
    {
        $request = new Request();
        $request->setMethod('PUT');
        $request->getHeaders()->addHeaderLine(
        ...
        $request->setContent(file_get_contents(__DIR__ . '/TestAsset/multipart-form-data-array-with-file-input.txt'));
        $event = new MvcEvent();
        $event->setRequest($request);
        $event->setRouteMatch($this->createRouteMatch([]));
        $listener = $this->listener;
